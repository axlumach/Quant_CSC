...
def main():
    ...
    data = {
        ...
        'Idade': [24, 27, 22, 32],
        'Cidade': ['New York', 'Los Angeles', 'Chicago', 'Houston']
    }
    ...
    st.write(df)
    # Título da aplicação
    st.title('Exemplo de Tabela Bonita com Streamlit')

    # Estilização do DataFrame usando o método st.dataframe()
    st.dataframe(df.style.set_table_styles(
        ...
        'border-color': 'white',
        'font-family': 'Arial'
    }).set_caption('Tabela de Exemplo'))

    # Outra opção: usando st.table() para tabelas estáticas (menos interativas)
