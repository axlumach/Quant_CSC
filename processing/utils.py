...
def portfolio_uploaded_checking(reference_date, portfolio):
    ...
    if portfolio in ['HGRIFFO63639', 'HGRIFFO67507', 'HGRIFFO67407'] :
        query = f"SELECT EXISTS (SELECT * FROM sch_hg.patrimonio_cotas c WHERE c.data_posicao = %s AND c.carteira = %s);"
    else:    
        query = f"SELECT EXISTS (SELECT * FROM sch_reag.patrimonio_totais p WHERE p.data = %s AND p.carteira = %s);"
    
    # reference_date = reference_date.astype('M8[D]').astype(dt.date)
    
    # Executar a query
    cursor.execute(query, (reference_date, portfolio))
